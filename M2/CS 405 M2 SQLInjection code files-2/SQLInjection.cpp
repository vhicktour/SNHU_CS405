...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Code added by VUdeh: Regular expression to detect common SQL injection patterns
  std::regex sql_injection_pattern(".*(or\\s+\\d+=\\d+|or\\s+\\'[^']*'='[^']*').*", std::regex::icase);

  // Code added by VUdeh: Check if the input SQL matches the injection pattern
